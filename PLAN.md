...
|---|---------|--------|--------|
| 12 | **Skybridge = TypeScript/React framework, not our deploy target.** | Deploy the Python MCP server on **Alpic** (GitHub-connect + `alpic.json`). Keep `mcp.run(transport="streamable-http")` — Alpic greps for it; absence defaults to stdio (wrong for remote). Bind `0.0.0.0`, read `PORT`. | github.com/alpic-ai/skybridge · docs.alpic.ai/build-deploy/builds |
